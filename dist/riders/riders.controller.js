...
let RidersController = class RidersController {
    ...
    async updateMyStatus(req, status) {
        const authReq = req;
        const rider = await this.ridersService.findOneByUserId(authReq.user.userId);
        if (!rider) {
            throw new common_1.ForbiddenException('User does not have a rider profile');
        }
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], RidersController.prototype, "updateMyStatus", null);
__decorate([
